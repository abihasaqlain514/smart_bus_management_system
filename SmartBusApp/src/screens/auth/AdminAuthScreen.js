...
export default function AdminAuthScreen() {
  ...
  const [regForm,   setReg]   = useState({
    full_name: '', email: '', password: '', phone_number: '+92', department: '',
  });
  ...
  const doRegister = async () => {
    ...
    if (!/[A-Z]/.test(password) || !/[a-z]/.test(password) || !/[0-9]/.test(password))
      return Alert.alert('Password error', 'Password must have uppercase, lowercase, and a digit.');

    setBusy(true);
    try {
      await authApi.adminRegister({ ...regForm, email, password, phone_number: phone });
      const { data } = await authApi.adminLogin({ email, password });
      await login({
        access_token: data.access_token,
        role:         'admin',
  ...
  return (
    <ScrollView style={styles.root} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      <View style={[styles.tabs, { backgroundColor: '#F3E5F5' }]}>
      ...
      {tab === 'login' ? (
        <>
          ...
          <Btn title="Login as Admin" onPress={doLogin} loading={busy} color={C} />

          <View style={styles.hintBox}>
            <Text style={[styles.hintTitle, { color: C }]}>Seeded admin account</Text>
            <Text style={styles.hint}>admin@smartbus.com  →  Admin@1234</Text>
            <Text style={[styles.hint, { marginTop: 4, color: COLORS.warning }]}>
              Note: 3 wrong attempts locks the account for 15 min.
            </Text>
          </View>
        </>
      ) : (
        <>
          <Input label="Full Name *"          value={regForm.full_name}    onChangeText={(v) => setReg({ ...regForm, full_name: v })}    autoCorrect={false} placeholder="Muhammad Hassan" />
          ...
          <Input label="Password *"           value={regForm.password}     onChangeText={(v) => setReg({ ...regForm, password: v })}     secureTextEntry autoCapitalize="none" autoCorrect={false} placeholder="Upper + lower + digit required" />
          <Input label="Phone * (+92...)"     value={regForm.phone_number} onChangeText={(v) => setReg({ ...regForm, phone_number: v })} keyboardType="phone-pad" />
          <Input label="Department (optional)" value={regForm.department}  onChangeText={(v) => setReg({ ...regForm, department: v })}  placeholder="Transport Management" />
          <Btn title="Register as Admin" onPress={doRegister} loading={busy} color={C} />
        </>
      )}
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  eyeText:   { fontSize: 22 },
  hintBox:   { marginTop: 20, backgroundColor: '#F3E5F5', padding: 14, borderRadius: 10 },
